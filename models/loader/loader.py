...
class LoaderCheckPoint:
    ...
    def _load_model_config(self, model_name):
        ...
        # model_config = AutoModel.from_pretrained("../model/chatglm-6b", trust_remote_code=True, device_map='auto')
        # model_config = PeftModel.from_pretrained(model_config, "../ChatGLM-Tuning/output").half()
        tokenizer = AutoTokenizer.from_pretrained("../ChatGLM2-6B/model/chatglm2-6b",trust_remote_code=True,)
        model = AutoModel.from_pretrained("../ChatGLM2-6B/model/chatglm2-6b", trust_remote_code=True, device_map='auto')#方法通常用于加载预训练的模型权重和参数
        model = PeftModel.from_pretrained(model, "../ChatGLM-Efficient-Tuning/examples/cognition").half()#原来是cognition,cognition5是新的微调的
        # model = PeftModel.from_pretrained(model, "../ChatGLM-Efficient-Tuning/src/ChatZhuanzhi_KBQA").half()
    ...
    def reload_model(self):
        self.unload_model() #调用unload_model方法卸载当前模型。
        ...
        self.model, self.tokenizer = self._load_model_config(self.model_name)# 调用_load_model_config方法加载模型和分词器，
...
#self.model.eval()是一个PyTorch中的方法，用于将模型设置为评估模式。在评估模式下，模型会禁用一些训练过程中的特定操作，例如批量归一化和随机丢弃（dropout），以确保模型在测试或推断阶段的行为一致。
#调用self.model.eval()会影响模型的行为，使其在前向传播过程中不进行训练，而是只计算输出。这在需要对模型进行推断或测试时很有用。
#通常，在执行模型推断之前，建议将模型设置为评估模式，以确保输出的一致性和可靠性。