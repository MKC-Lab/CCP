...
async def list_docs():
    ...
    return List[str](filepath)

async def local_doc_chat(#加载知识库
    ...
    if not os.path.exists(vs_path):
        ...
        prompt_content = f"{html.escape(prompt)}"

        return ChatMessage(
            question=question,
            response=response["result"],
            history=history,
            source_documents=[prompt_content],
        )


async def chat(#没加载知识库的问答
        question: str = Body(..., description="Question", example="工伤保险是什么？"),
        history: List[List[str]] = Body(
            [],
            description="History of previous questions and answers",
...
def api_start(host, port):
    ...
    llm_model_ins = shared.loaderLLM()#加载大模型
    llm_model_ins.set_history_len(LLM_HISTORY_LEN)#历史长度3
    ...
    app.post("/chat", response_model=ChatMessage)(chat)#没加载数据库的聊天

    app.post("/local_doc_qa/local_doc_chat", response_model=ChatMessage)(local_doc_chat)#加载知识库
