...
class LocalDocQA:
    ...
    def get_knowledge_based_answer(self, query, vs_path, train_context, knowledge_dataset,chat_history=[], streaming: bool = STREAMING):#得到答案
        # vector_store = load_vector_store(vs_path, self.embeddings)
        # FAISS.similarity_search_with_score_by_vector = similarity_search_with_score_by_vector
        # vector_store.chunk_size = self.chunk_size#300。匹配后单段上下文长度（在匹配的文本分句长度前后扩充至CHUNK_SIZE字）
        # vector_store.chunk_conent = self.chunk_conent#true 在相似性搜索期间返回块的内容。
...
#########
        ...
        print(f"\n提示模板为:\n {prompt}")
########
        for answer_result in self.llm.generatorAnswer(prompt=prompt, history=chat_history,streaming=False):#原来streaming=streaming
